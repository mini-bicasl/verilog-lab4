...
- [ ] `ddr4_init_fsm`: Power-up and initialization FSM (RESET_N, CKE, MRS, ZQCL sequence per JESD79-4 §3.3)
- [ ] `ddr4_mode_reg`: MR0–MR6 shadow registers and MRS payload encoder
- [ ] `ddr4_timing_engine`: JEDEC timing constraint down-counters (tRCD, tRP, tRAS, tRC, tCCD, tRRD, tWR, tWTR, tRTP, tFAW, tRFC, tMOD, tZQ)
- [ ] `ddr4_refresh_ctrl`: tREFI countdown; all-bank REF, Fine Granularity Refresh (FGR ×2/×4), Per-Bank Refresh (PBR), self-refresh entry/exit
- [ ] `ddr4_cmd_scheduler`: Per-bank open/closed-page FSM, command queue, rank interleaving, refresh arbitration

### Testbenches

- [ ] `ddr4_init_fsm`: TB — verifies RESET_N deassertion timing, MRS sequence order and MR values, ZQCL, init_done assertion
- [ ] `ddr4_mode_reg`: TB — verifies correct MR0–MR6 encoding for representative CL/CWL/RTT combinations
- [ ] `ddr4_timing_engine`: TB — verifies that timing_ok bits de-assert after command and re-assert after correct number of cycles for all 16 constraints
- [ ] `ddr4_refresh_ctrl`: TB — verifies tREFI expiry generates ref_req; FGR×2 halves effective period; PBR rotates through all 16 banks; sr_active asserted after sr_req
- [ ] `ddr4_cmd_scheduler`: TB — issues READ and WRITE requests; verifies ACT→RD/WR→PRE sequence; verifies refresh preemption; verifies rank interleaving

### Documentation

- [ ] `docs/ddr4_init_fsm.md`: FSM state diagram, port table, timing diagram for init sequence
- [ ] `docs/ddr4_mode_reg.md`: MR0–MR6 bit-field map, encoding tables
- [ ] `docs/ddr4_timing_engine.md`: Constraint table, counter architecture, timing_ok semantics
- [ ] `docs/ddr4_refresh_ctrl.md`: Refresh mode comparison, self-refresh flow
- [ ] `docs/ddr4_cmd_scheduler.md`: Bank FSM states, arbitration policy, refresh integration
